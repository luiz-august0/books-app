...
public enum EnumDateFormat {
    ...
    EXTENSIVE(newThreadLocalSimpleDateFormat("MMMM d, yyyy", Locale.ENGLISH)),
    ...
    private static ThreadLocal<SimpleDateFormat> newThreadLocalSimpleDateFormat(final String frmtString, Locale locale) {
        return ThreadLocal.withInitial(() -> {
            return new SimpleDateFormat(frmtString, locale);
        });
    }
